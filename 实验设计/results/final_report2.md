...
| 参数 | 说明 |
|---|---|
| 基线方法 | Transformer PPO / Linear PPO / Greedy baseline |
| 策略检查点 | `ppo_policy_transformer.pth`，`checkpoints/`（Linear） |
| 客户数量 | 8（在本实例中平均访问客户数为 8） |
| 问题规模 | 实验实例以小规模节点为主（示例中客户+仓库+站点 ~ 10 节点级别） |
| 充电站 | 实验包含至少一个充电站（详见 `env.py` 配置） |
| 能耗模型 | 线性距离模型：$\Delta SoC = \eta \cdot d_{ij}$（恒速假设） |
| 充电策略 | 到站即全量充电（Full recharge policy） |
| 动作空间 | 仅选择下一个节点（节点选择） |
| 评估次数 | Transformer: 5 runs；Linear: 5 runs；Greedy: 1 run（CSV 已保存） |
| 评估类型 | 确定性评估（deterministic evaluations，详见 CSV） |
| 报告指标 | 总奖励、最终 SoC、完成时间（final_clock）、步骤数、访问计数 |

（CSV 文件包含每次评估的逐条记录，可用于计算置信区间与成对比较。）
